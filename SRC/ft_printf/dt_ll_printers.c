...
/*   Updated: 2018/01/18 19:54:59 by slynn-ev         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	pf_dot_ll_hex_upper(long long num, char *flags, int mod[2])
{
	...
	count = 0;
	precision = (ft_strrchr(flags, '#') && num != 0) ? 2 : 0;
	num_l = (num >= 0) ? get_num_length(num, HEXA) :
...
int	pf_dot_ll_binary(long long num, char *flags, int mod[2])
{
	char	space_type;
	int		count;
	int		num_l;
	...
	count += non_dec_dot_leftadj_print1(num_l, mod[1], HEXA, 0);
	ft_putbase(num, BINARY, sizeof(long long) * 8, 0);
	return (non_dec_dot_leftadj_print2(num_l, mod[0], count) + num_l);
}
